						...
						<div id="thong-tin-phim" class="tabcontent">
							<div class="film-info">
								<div class="form-group mt-15">
									...
									<div class="form-item">
										<label for="">Tên phim</label> <input type="text" name="name" placeholder=" Tên"
											value="${m.name}">
									</div>
									<div class="form-item">
								...
								<div class="form-group mt-15">
									...
									<div class="form-item">
										<label for="">Tổng số tập phim</label> <input type="text" name="episode_total"
											value="${m.episode_total}">
									</div>
								</div>
								<div class="form-group mt-15">
									<div class="form-item">
										<label for="">Ngôn ngữ</label> <input type="text" name="language"
											placeholder=" Ngôn ngữ" value="${m.language}">
									</div>
									<div class="form-item">
									...
									<c:forEach items="${requestScope.type}" var="t">
										...
										<div class="form-check mt-8">
											<c:if test="${m.type.id == type}">
												<input type="radio" value="${t.id}" name="type" checked="checked">
												<label>${t.name}</label>
											</c:if>
											<c:if test="${m.type.id != type}">
									...
									<c:forEach items="${requestScope.status}" var="s">
										...
										<div class="form-check mt-8">
											<c:if test="${m.status.id == status}">
												<input type="radio" value="${s.id}" name="status" checked="checked">
												<label>${s.name}</label>
											</c:if>
											<c:if test="${m.status.id != status}">
									...
									<div class="form-check-row">
										<c:forEach items="${requestScope.c}" var="r">
											...
											<div class="form-check mt-8">
												...
												<input type="checkbox" value="${r.id}" name="category" ${check}>
												<label>${r.name}</label>
									...
									<table class="crud-table " id="episode_table">
										...
										<tbody id="table-body">
											<c:forEach items="${m.getEpisodes()}" var="e">
												<tr>
													<td><input type="text" value=" ${e.name}" name="episode_name"></td>
													<td><input type="text" name="episode_slug" value=" ${e.slug}"></td>
								...
								<div class="mt-15">
									<div class="form-check mt-8">
										...
										<c:if test="${m.is_shown_in_theater == 1}">
											<input type="checkbox" name="is_shown_in_theater" checked="checked">
											<label>Phim
												chiếu rạp</label>
										</c:if>
									...
									<div class="form-check mt-8">
										<c:if test="${m.is_sensitive_content == 1}">
											<input type="checkbox" name="is_sensitive_content" checked="checked">
											<label>Cảnh
